...
options read_input(string filename) {
  ...
  try {
    ...
    opt.N_sim_between = cfg.lookup("N_sim_between");

    opt.Dir_Name = cfg.lookup("Dir_Name");
  ...
  try {
    opt.EVOLUTION_TYPE = cfg.lookup("EVOLUTION_TYPE");
  ...
  try {
    opt.EVOLUTION_EXP_CUTOFF = cfg.lookup("EVOLUTION_EXP_CUTOFF");
  } catch (const SettingException &errortype){
    cerr << "Error, the setting " << errortype.getPath() << " is of the wrong type." << endl;
    exit(EXIT_FAILURE);
  }
  ...
  try {
    opt.EVOLUTION_POWER_EXP = cfg.lookup("EVOLUTION_POWER_EXP");
  } catch (const SettingException &errortype){
    cerr << "Error, the setting " << errortype.getPath() << " is of the wrong type." << endl;
    exit(EXIT_FAILURE);
  }
